...
- 2F (수평 필렛) 는 위빙 평면이 비교적 단순해 지령 전류와 정상 스틱아웃 시 측정 전류가 거의 일치 → `+0`.
- 3F (수직 자세) 는 위빙 중 와이어 송급 / 아크 거동이 살짝 달라 정상 스틱아웃에서도 보통 **+3 A 가량 높게 측정** → `+3`.

이 `+α` 값은 어디까지나 **현장에서 측정해 끼워 맞춘 경험치** 이며, 실제로는 다음과 같은 리스크가 있습니다.

- **전압이 바뀌면 `+α` 도 바뀝니다.** 같은 지령 전류라도 전압 셋팅이 다르면 정상 스틱아웃에서의 전류 자체가 달라지므로, `+3` 이 더 이상 맞지 않을 수 있습니다.
- **용접기 간 편차가 있으면 `+α` 가 용접기마다 달라집니다.** 즉 “3F 면 +3” 같은 한 줄짜리 규칙이 모든 용접기에 통하지 않게 됩니다.
