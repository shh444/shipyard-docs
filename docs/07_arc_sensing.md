...
- **용접기 간 편차가 있으면 `+α` 가 용접기마다 달라집니다.** 즉 “3F 면 +3” 같은 한 줄짜리 규칙이 모든 용접기에 통하지 않게 됩니다. 다만 현대 PNS 용접기의 경우 칼리브레이션을 해서인지 해당 부분에서 문제가 되지 않았었습니다. 또한 현중의 경우 +- 를 하지 않아도 항상 동일한 stick이 되도록 용접기 쪽에서 맞췄습니다.
