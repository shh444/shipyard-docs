...
실제 코드에서는 **첫 사이클이 끝나기 전(`cycle_done == False`)에 한 번만** `Standard_Arc_Current` 를 확정합니다.

```python
if cycle_done == False:
    if th_arc_sen[5] != 0:
        # (1) 사용자가 직접 기준 전류를 지정한 경우 — 최우선
        Standard_Arc_Current = th_arc_sen[5]
    else:
        # (2) 미지정 시 자세별 기본 보정값을 자동 산출
        if th_par_2f:
            # 2F (수평 필렛)
            Standard_Arc_Current = current_to_welding_m
        else:
            # 3F (수직 상진/하진 등)
            Standard_Arc_Current = current_to_welding_m + 3
        end
    end
...
| 우선순위 | 조건 | 기준 전류 | 비고 |
| ---: | --- | --- | --- |
| 1 | `th_arc_sen[5] != 0` | `th_arc_sen[5]` | 펜던트 / 외부에서 직접 지정 |
| 2 | 2F (`th_par_2f == True`) | `current_to_welding_m` | 지령 전류 그대로 |
| 3 | 3F (`th_par_2f == False`) | `current_to_welding_m + 3` | **+3 A 보정** |

#### 자세에 따른 `+α` 보정의 의미

`th_arc_sen[5]` 미지정 시 자동값은 **자세 + 지령 전류** 의 함수입니다.

- 현재 2F (수평 필렛) 는 설정이 안되있음 (혹은 +0도 괜찮음.) → `+0`.
- 3F (수직 자세) 는 위빙 중 와이어 송급 / 아크 거동이 살짝 달라 정상 스틱아웃에서도 보통 **+3 A 가량 높게 측정** → `+3`.

이 `+α` 값은 어디까지나 **현장에서 측정해 끼워 맞춘 경험치** 이며, 실제로는 다음과 같은 리스크가 있습니다. 해당 내용은 반드시 레코딩을 통해 20mm일때 +- 전류값을 얼마나 해야하는지 보고 정확히 넣어야 합니다.
 
- **전압이 바뀌면 `+α` 도 바뀔 수 있습니다.** 같은 지령 전류라도 전압 셋팅이 다르면 정상 스틱아웃에서의 전류 자체가 달라질 수 있지만, 경험상 비슷한 조건을 사용해서 인지 한국에서는 크게 문제가 되지 않았습니다.

- **용접기 간 편차가 있으면 `+α` 가 용접기마다 달라집니다.** 즉 “3F 면 +3” 같은 한 줄짜리 규칙이 모든 용접기에 통하지 않게 됩니다. 다만 현대 PNS 용접기의 경우 칼리브레이션을 해서인지 해당 부분에서 문제가 되지 않았었습니다. 또한 현중의 경우 +- 를 하지 않아도 항상 동일한 stick이 되도록 용접기 쪽에서 수정을 해서 맞췄습니다.

#### 가장 깔끔한 해법 — 용접기 캘리브레이션

따라서 운용상 가장 이상적인 셋업은 다음과 같습니다.

> **“스틱아웃 20 mm 에서 출력 전류가 지령 전류와 동일하도록 각 용접기를 캘리브레이션해 두는 것”**

이렇게 하면 자세에 따른 `+α` 자체가 필요 없어지고, 어떤 용접기를 물려도 `Standard_Arc_Current = current_to_welding_m` 로 단순화할 수 있습니다. 자세별 분기, `+3` 같은 보정값, 용접기별 편차 모두 한 번에 사라집니다.

#### 현장 운용 경험

- **하나의 라인 안에서는** 동일한 `+α` 값으로 모든 용접기를 운용해도 문제가 없다는 점을 PNS 현장에서 확인했습니다. 캘리브레이션이 잘 되어 있어 용접기 간 편차가 거의 없는 것으로 추정됩니다.
- 반면 **조선소(현장)가 달라지면 `+α` 값이 다르게 잡힙니다.** 접지 상태 / 모재 소재 / 와이어 송급 환경 등이 달라 같은 지령 전류에서도 정상 스틱아웃 시 측정되는 전류가 달라지기 때문으로 보입니다.

> 그래서 **사이트 첫 셋업 시에는 반드시 `+α` 를 다시 측정** 하고, 필요하면 `th_arc_sen[5]` 로 직접 값을 지정해 자세 분기 자체를 우회하는 것이 안전합니다.
